...
export const useBatchBookShifts = () => {
  ...
  return useMutation({
    mutationFn: async (shiftIds: string[]) => {
      try {
        // Check if shift IDs are valid
        if (!shiftIds || !Array.isArray(shiftIds) || shiftIds.length === 0) {
          throw new Error('Inga giltiga pass valda');
        }
        ...
        for (const shiftId of shiftIds) {
          try {
            if (!shiftId) {
              console.error('Invalid shift ID encountered in batch');
              errors.push({ shiftId: 'unknown', error: new Error('Ogiltig pass-ID') });
              continue;
            }
        ...
        console.error('Error in batch booking mutation:', error);
        throw error;
      }
    },
    onSuccess: (data) => {
